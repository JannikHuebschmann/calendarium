...
public class TerminObjekt extends EintragsObjekt implements Shared {
	...
	private void create(boolean border, int fontSize) {
		...
		// Owner?
		if (termin.getOwner() != null
				&& termin.getOwner().getID() == Data.user.getID())
			isBerechtigt = true;
		...
		if (isBerechtigt) {
			...
			if (border) {
				typPane.setBorder(new CustomBorder(Color.black, false, true,
						false, false));
			}
			...
			farbe.setBackground(Data.typen.getUserTyp(termin.getTyp())
					.getFarbe());
