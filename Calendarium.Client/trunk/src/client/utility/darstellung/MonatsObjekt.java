...
public class MonatsObjekt extends JPanel implements ActionListener,
		ItemListener, Shared {
	...
	private JPanel createMtSelect(int mt, int yr) {
		...
		JButton b1 = new JButton(client.Client.loadImageIcon("left.gif"));
		b1.setMargin(new Insets(0, 0, 0, 0));
		b1.setAlignmentY((float) 0.5);
		b1.setRolloverEnabled(true);
		b1.setRolloverIcon(client.Client.loadImageIcon("leftRollover.gif"));
		b1.setActionCommand("previousMonth");
		b1.addActionListener(this);

		JButton b2 = new JButton(client.Client.loadImageIcon("right.gif"));
		b2.setMargin(new Insets(0, 0, 0, 0));
		b2.setAlignmentY((float) 0.5);
		b2.setRolloverEnabled(true);
		b2.setRolloverIcon(client.Client.loadImageIcon("rightRollover.gif"));
		...
		for (int i = 0; i < 12; i++) {
			monthCombo.addItem(MONTHNAMESLONG[i]);
		}
		...
		for (int i = 0; i < 7; i++) {
			for (int j = 0; j < 7; j++) {
				if (i == 0) {
					...
					JLabel text = new JLabel(DAYNAMESSHORT[j]);
					...
					if ((feiertag = Data.feiertage.getFeiertagByDate(new Datum(
							day))) != null
							|| j == 6) {
						// Feiertag
						...
						if (feiertag != null) {
							days[index].setToolTipText(feiertag
									.getBezeichnung());
						}
					...
					if (compareDate(day, cal)) { // aktuelles Datum
						days[index].setBorder(new CompoundBorder(
								new BevelBorder(BevelBorder.RAISED),
								new LineBorder(Color.blue, 1)));
					}

					days[index].setMargin(new Insets(0, 0, 0, 0));
					days[index].getAccessibleContext()
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals("previousMonth")) {
			...
			int mo = Integer.valueOf(
					days[index].getAccessibleContext()
							.getAccessibleDescription()).intValue();
			if (mt != mo) {
				if (index < 7) {
					mt--;
					if (mt < 0) {
						...
						yr--;
						yearCombo
								.setSelectedIndex(yearCombo.getSelectedIndex() - 1);
					}
				} else {
					mt++;
					if (mt > 11) {
