...
public class TeilnehmerPanel extends BasicSelection implements Shared {
	// Teilnehmer
	...
	int isBerechtigt(Gruppe gruppe) {
		...
		if (personen.size() > 0) {
			...
			String text, msg = "Sie haben keine Eintragsberechtigung fuer Einträge vom Typ "
					+ c
					+ recht.getEintragsTyp().getBezeichnung()
					+ c
					+ " bei:\n\n";
			...
			if (selectedValue != null) { // Trotzdem hinzufuegen
	...
	void removeGruppe(int rowIndex) {
		if (failedGruppen != null) {
			failedGruppen.removeElement((Gruppe) gruppenTable.getValueAt(
					rowIndex, 0));
		}
	...
	void setCheckboxes(int column, int typ) {
		...
		if (typ == 0) {
			...
			for (int i = 0; i < count; i++) {
				gruppenTableModel.setValueAt(new Boolean(
						chkState[column - 2][typ]), i, column);
			}

		} else {
			int count = personenTableModel.getRowCount();
			for (int i = 0; i < count; i++) {
				personenTableModel.setValueAt(new Boolean(
						chkState[column - 2][typ]), i, column);
			}
		}
	}
			...
			if (nfkt[i]) {
				if (!column[i]) {
					...
					for (int j = 0; j < count; j++) {
						gruppenTableModel.setValueAt(new Boolean(true), j,
								i + 2);
					}

					count = personenTableModel.getRowCount();
					for (int j = 0; j < count; j++) {
						personenTableModel.setValueAt(new Boolean(true), j,
								i + 2);
					}
				}
	...
	void getAttribute() {
		...
		for (i = 0; i < rows; i++) {
			...
			for (j = 2; j < 5; j++) {
				if (column[j - 2]) {
					nfkt[count] = ((Boolean) personenTableModel
							.getValueAt(i, j)).booleanValue();
					count++;
				}
			}
