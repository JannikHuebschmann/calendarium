...
class TerminAllgPanel extends AllgemeinPanel implements FocusListener {
	...
	JPanel createDauer() {
		...
		pane.setBorder(new CompoundBorder(new EmptyBorder(0, 3, 0, 3),
				new CustomBorder(SystemColor.controlHighlight, false, false,
						true, true)));
	...
	JPanel createDate() {
		...
		date1.add((timePanel[0] = new TimePanel()).getGUI());// NULLPOINTER
		// 15.5.2k4
	...
	void fill(Termin t) {
		...
		if (termin.getBeginn().isCorrectDate()) {
			...
			if (termin.getBeginn().isCorrectTime()) {
				timePanel[0].getZeitfeld()
						.setTime(termin.getBeginn().getTime());
			}
		}
		...
		if (termin.getEnde().isCorrectDate()) {
			...
			if (termin.getEnde().isCorrectTime()) {
				...
				if (termin.getBeginn().isCorrect()) { // Dauer
					dauer.setValue(termin.getEnde().getHoursBetween(
							termin.getBeginn()));
				}
			}
		}
	...
	public void focusLost(FocusEvent e) {
		...
		if (e.getSource() == dauer) {
			...
			if (termin.getBeginn().isCorrect()) {
				...
				datePanel[1].getDatumsfeld()
						.setDate(termin.getEnde().getDate());
				timePanel[1].getZeitfeld().setTime(termin.getEnde().getTime());
			}
