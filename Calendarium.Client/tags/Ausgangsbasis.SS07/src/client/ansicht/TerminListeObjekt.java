	...
	class TerminScrollListe extends JScrollPane {
		...
		void create() {
			...
			for (int i = 0; i < 24; i++) {
				...
				headers.setBorder(new CustomBorder(Color.black, true, false,
						true, true));
				headers.setLayout(new BoxLayout(headers, BoxLayout.X_AXIS));

				if (i < 10) {
					x = small ? 7 : 16;
					headers.add(Box.createRigidArea(new Dimension(x,
							HOUR_HEIGHT)));
				} else {
					x = small ? 0 : 8;
					headers.add(Box.createRigidArea(new Dimension(x,
				...
				if (!small)
					headers.add(Box.createRigidArea(new Dimension(5,
							HOUR_HEIGHT)));
				...
				for (int j = 0; j < 2; j++) {
					...
					if (j == 0) {
						...
						mLabel.setBorder(new CustomBorder(Color.black, true,
								true, false, false));
					} else {
						mLabel = new JLabel(":30");
						mLabel.setBorder(new CustomBorder(Color.black, false,
								true, true, false));
					}
					...
					x = small ? 12 : 20;
					mLabel.setPreferredSize(new Dimension(x,
							HOUR_HEIGHT / 2 + 1));
					...
					zeile.setPreferredSize(new Dimension(30,
							HOUR_HEIGHT / 2 + 1));
				...
				for (j = 0; j < 48; j++) {
					if (grid[j][i] != 0) {
						if (bHeight > 0
							...
							TerminObjekt tObj = new TerminObjekt(t,
									sicht.openCal, true, 10);
						...
						nothing.setBorder(new CustomBorder(Color.black, true,
								false, true, false));
						nothing.setMaximumSize(new Dimension(500,
								HOUR_HEIGHT / 2 + 1));
