...
public class Datum implements Serializable, data.Shared
{
    private static final long serialVersionUID = -3883599196032987628L;
    ...
    private int getMinuten(String time)
	{
	    ...
	    return Integer.valueOf(time.substring(0, pos)).intValue() * 60 +
	           Integer.valueOf(time.substring(pos + 1, time.length())).intValue();
	}
	...
	public long getDatum()
	{
	    ...
	    if (time.equals("99:99")) cal = new GregorianCalendar(getYear(),getMonth()-1,getDay());
	    else cal = new GregorianCalendar(getYear(),getMonth()-1,getDay(),getHour(),getMin());
	...
	{   return getDaysSinceUrsprung(d.getDate()) - getDaysSinceUrsprung(this.date);
	}
    ...
    public boolean isCorrectDate()
	{
	    ...
	    if(dy != 99 && mt != 99 & yr != 9999 &&
	       dy > 0 && mt > 0 && yr > 0 && mt < 13 &&
	       dy <= calcMonthDays(mt, yr)) return true;
	    else return false;
	}
	...
	{
	    if(getHour() < 24 && getMin() < 60) return true;
	    else return false;
	}
	...
	public Datum addDauer(double dauer) // dauer in Stunden
	{
	    ...
	    double summe = getDaysSinceUrsprung(date) * 1440
	                   + getMinuten(time) + dauer * 60;
	    ...
	    return new Datum(String.valueOf(dy) + DATETOKEN + String.valueOf(mt) + DATETOKEN + String.valueOf(yr),
	                     String.valueOf(hr) + TIMETOKEN + String.valueOf(min));
	}
    ...
    public void add(int nr)
    {
	    ...
	    date = (dy < 10 ? "0" + dy : "" + dy) + "." + (mt < 10 ? "0" + mt : "" + mt) + "." + yr;
    }
