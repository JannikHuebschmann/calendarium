...
public abstract class Server implements PersonenIF, TermineIF {
    ...
    static {
        ...
        try {
            FileInputStream in = new FileInputStream("SWTKal.Base/branches/SVN.u.Build.Aufgabe/src/swtkalServerProperties.xml");
            serverProperties.loadFromXML(in);
            in.close();
        } catch (IOException e) {
            e.printStackTrace();
            try {    // write current properties to file swtkalServerProperties.xml
