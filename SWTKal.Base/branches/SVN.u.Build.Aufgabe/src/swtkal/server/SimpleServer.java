...
//	protected Map<String, Map<String, Vector<Termin>>> besitzerTermine;
    ...
    protected SimpleServer() {
        ...
        try {
            ...
            insert(new Termin(p, "1. Testtermin", "Dies ist der Langtext zum 1. Testtermin",
                    new Datum(new Date()), new Datum(new Date()).addDauer(1), 1));
            insert(new Termin(p, "2. Testtermin", "Dies ist der Langtext zum 2. Testtermin",
                    new Datum(new Date()).addDauer(1.5), new Datum(new Date()).addDauer(2.5), 2));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public boolean isPersonKnown(String kuerzel) {
        return personen.containsKey(kuerzel);
    }
    ...
    public void insert(Termin termin) throws TerminException {
        ...
        for (Termin t : extractTerminList(teilnehmerTermine)) {
            if (t.getId().equals(termin.getId())) {
                throw new TerminException("Id is already taken!");
            }
        }
...
//		insert(termin, termin.getBesitzer(), besitzerTermine);
    ...
    private void insert(Termin termin, Person p, Map<String, Map<String, Vector<Termin>>> map) {
        termin.setId(extractTerminList(map).size());
        if (!map.containsKey(p.getKuerzel())) {    // first appointment for this person
        ...
        } else if (!map.get(p.getKuerzel()).containsKey(termin.getBeginn().getDate())) {    // first appointment for this date
            Vector<Termin> vector = new Vector<Termin>();
            vector.add(termin);                                    // only one appointment
            map.get(p.getKuerzel()).put(termin.getBeginn().getDate(), vector);
        } else {    // additional appointment for this person and this date
...
//		delete(termin, termin.getBesitzer(), besitzerTermine);
    ...
    private List<Termin> extractTerminList(Map<String, Map<String, Vector<Termin>>> nestedMap) {
        List<Termin> terminList = new ArrayList<>();

        for (Map<String, Vector<Termin>> innerMap : nestedMap.values()) {
            for (Vector<Termin> terminVector : innerMap.values()) {
                terminList.addAll(terminVector);
            }
        }
